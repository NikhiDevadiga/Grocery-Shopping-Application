...
const Navbar = () => {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      if (searchValue.trim()) {
        axios.get(`https://manglore-store-dy2a.onrender.com/api/product?query=${searchValue}`)
  ...
  const handleProfileUpdate = async () => {
    try {
      ...
      await axios.put(`https://manglore-store-dy2a.onrender.com/api/update/${user._id}`, editProfileData);
