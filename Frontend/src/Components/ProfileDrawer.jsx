...
const ProfileDrawer = ({
  ...
  const fetchUserDetails = async () => {
    try {
      const res = await axios.get("https://manglore-store-dy2a.onrender.com/api/users/list");
  ...
  const fetchOrders = async () => {
    try {
      ...
      const res = await axios.get(`https://manglore-store-dy2a.onrender.com/api/user/${user._id}`);
      setOrders(res.data);
    } catch (error) {
