...
export default function CartDrawer({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchAddresses = async () => {
      try {
        const res = await axios.get(`https://manglore-store-dy2a.onrender.com/api/address/${user._id}`);
  ...
  const saveAddressToBackend = async () => {
    ...
    try {
      const response = await axios.post(
        `https://manglore-store-dy2a.onrender.com/api/address/${user._id}`,
        address,
        {
          headers: {
  ...
  const handleProceedPayment = async () => {
    ...
    const saveOrderToBackend = async (extra = {}) => {
      ...
      try {
        ...
        const response = await axios.post('https://manglore-store-dy2a.onrender.com/api/admin/createOrders', order);
      } catch (err) {
