...
const Product = () => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        if (isSingleProductPage && id) {
          const res = await axios.get(`https://manglore-store-dy2a.onrender.com/api/product/by-id/${id}`);
          const product = res.data?.data || res.data;
          const cleaned = cleanProductData(product && product._id ? [product] : []);
          setProducts(cleaned);
        } else if (subcategoryId) {
          const res = await axios.get(`https://manglore-store-dy2a.onrender.com/api/product/${subcategoryId}`);
          const data = res.data.data;
          const cleaned = cleanProductData(Array.isArray(data) ? data : [data]);
          setProducts(cleaned);
        }
      } catch (err) {
  ...
  const formatImagePath = (path) =>
    `https://manglore-store-dy2a.onrender.com/${path?.replace(/\\/g, "/")}`;
  ...
  const cleanProductData = (products) => {
    ...
    return products.map(product => {
      const validTillDate = product.offer?.validTill ? new Date(product.offer.validTill).setHours(0, 0, 0, 0) : null;
      
      if (validTillDate && validTillDate < today) {
        return { ...product, offer: null }; // remove expired offer
      }
      return product;
    });
  };
